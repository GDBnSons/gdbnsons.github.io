...
const CURRENT={
  date:"2026-05-02",usdEur:0.853022,eurUsd:1.172303,
  totalUSD:378975,totalEUR:323274,
  fearGreed:31,btcPrice:78309,gdbC:34.9355,gdbS:13.4883,
  crypto:{total:195772,items:[{t:"BTC",qty:2.5,pa:63618.44,live:78309,val:195772,pnl:36726,pct:.2306}]},
  bank:{totalEUR:18874,breakdown:{BCI:4689,Bourso:13481,DeBlock:703}},
  stocks:{total:161077,items:[{t:"QQQ",cat:"Indices",qty:32,pa:611.92,live:674.15,val:21573,pnl:1991,pct:0.1017},{t:"AIA",cat:"Indices",qty:230,pa:108.2,live:125.28,val:28814,pnl:3928,pct:0.1579},{t:"JEDI",cat:"Indices",qty:210,pa:76.13,live:98.0,val:20580,pnl:4593,pct:0.2873},{t:"ROBO",cat:"Indices",qty:260,pa:73.65,live:81.94,val:21304,pnl:2155,pct:0.1126},{t:"XLE",cat:"Indices",qty:225,pa:51.0,live:58.85,val:13241,pnl:1766,pct:0.1539},{t:"OIH",cat:"Indices",qty:30,pa:374.8,live:444.73,val:13342,pnl:2098,pct:0.1866},{t:"AVIO",cat:"Picking",qty:300,pa:35.4037,live:37.162,val:11149,pnl:528,pct:0.0497},{t:"DJT",cat:"Picking",qty:800,pa:9.23,live:9.39,val:7512,pnl:128,pct:0.0173},{t:"GOLD",cat:"Or",qty:100,pa:177.369,live:181.707,val:18171,pnl:434,pct:0.0245},{t:"IBKR",cat:"Picking",qty:15.2762,pa:65.46,live:80.46,val:1229,pnl:229,pct:0.2291},{t:"EURO",cat:"Cash",qty:3550,pa:1.17,live:1.1723,val:4162,pnl:8,pct:0.002}]},
  alloc:[
    {n:"Bitcoin",pct:52.05,tgt:50,usd:194072,c:"#F7931A"},
    {n:"Indices",pct:31.32,tgt:22,usd:116766,c:"#1E40AF"},
    {n:"Actions",pct:5.29,tgt:8,usd:19718,c:"#0EA5E9"},
    {n:"Or",pct:4.99,tgt:5,usd:18622,c:"#EAB308"},
    {n:"Cash Dip",pct:1.29,tgt:10,usd:4815,c:"#10B981"},
    {n:"Cash Mat.",pct:5.06,tgt:5,usd:18857,c:"#6B7280"},
  ],
};
...
function applyPrices(prices, usdEur){
  ...
  /* GDB.C et GDB.S — prix proportionnel au mouvement BTC pour GDB.C,
     proportionnel au total actions pour GDB.S */
  const btcRatio = btcLive / CURRENT.btcPrice;
  const gdbC = parseFloat((CURRENT.gdbC * btcRatio).toFixed(4));
  const actionsRatio = stocksTotal > 0 ? stocksTotal / CURRENT.stocks.total : 1;
  const gdbS = parseFloat((CURRENT.gdbS * actionsRatio).toFixed(4));
...
const GIST_TOKEN = "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";
...
═══════════════════════════════════════════════════════════ */
function GdbCompareChart({eur, EFF}){
  const svgRef = useRef(null);
  const [tf, setTF] = useState("YTD");
  const [hover, setHover] = useState(null);
  const [full, setFull] = useState(false);

  // La valeur du portefeuille dans le graphique = exactement celle du header
  const portTodayEUR = CURRENT.totalEUR;  // 323,455 € hors immo (col AO)
  const portTodayUSD = CURRENT.totalUSD;  // 376,132 $ hors immo
  ...
  const tfCut = { "1W":cutoff(7), "1M":cutoff(31), "MTD":"2026-04-01", "YTD":"2026-01-01", "ALL":"2026-01-01" };
  const cut = tfCut[tf] || "2026-01-01";

  // GDB series: [date, gs$, gc$]  — 84 daily points
  const gSlice = GDBS.filter(r => r[0] >= cut && r[0] <= TODAY);
  // Portfolio base-100 (absolute €, jan 2026 = 326791)
  const pSlice = PORT_B100.filter(r => r[0] >= cut && r[0] <= TODAY);
  ...
  /* ── Portfolio → absolute values in selected currency ──
     PORT_B100 stores base-100 relative to jan 1 2026 total (326791€)
     actual_eur = b100/100 * 326791
     Last point forced to match header value exactly.
  ── */
  const pByDate = {};
  pSlice.forEach(r => { pByDate[r[0]] = r[1]; });

  // Compute absolute portfolio values per date
  const portAbs = dates.map(d => {
    const b = pByDate[d];
    if (!b) return null;
    const eur_val = Math.round(b / 100 * 313653);  // col AO base jan 2026
    return eur ? eur_val : Math.round(eur_val * CURRENT.eurUsd);
  });

  // Override last point to match header exactly
  if (portAbs.length > 0) portAbs[portAbs.length - 1] = portToday;

  // Rebase portfolio to 100 from slice start (for unified scale)
    ...
    <>
      ...
      {hover != null && (
        <div style={{
          ...
          <div style={{ fontSize: 11, color: "#fff", fontWeight: 800, width: "100%", textAlign: "center", marginBottom: 2 }}>
            {fmtDate(hDate)}
          </div>
          {[
      ...
      <div style={{ display: "flex", gap: 14, justifyContent: "center", marginTop: 2, paddingTop: 4, borderTop: `1px solid ${C.border}` }}>
        {[
          { color: C.orange, label: `GDB.C ${eur?"€"+((EFF||CURRENT).gdbC*(EFF||CURRENT).usdEur).toFixed(2):"$"+((EFF||CURRENT).gdbC).toFixed(2)}` },
          { color: C.blue,   label: `GDB.S ${eur?"€"+((EFF||CURRENT).gdbS*(EFF||CURRENT).usdEur).toFixed(2):"$"+((EFF||CURRENT).gdbS).toFixed(2)}` },
...
─────────────────────────────────────────────────────── */
function PerfStrip({eur}){
  const usd=!eur;
  // P&L in € always (portfolio is tracked in €), convert to $ if needed
  const rate = CURRENT.eurUsd; // 1€ = 1.1712$
  const pnl = (v) => eur ? v : Math.round(v * rate);
  const cur  = eur ? "€" : "$";
  const cells = [
    { label:"P&L 1J",  pnl:pnl(+4524),  pct:+0.0142 },
    { label:"P&L 1S",  pnl:pnl(+2304),  pct:+0.0072 },
    { label:"P&L 1M",  pnl:pnl(+26455), pct:+0.0890 },
  ];
  // GDB prices depuis GDBS (même source que l'onglet GDB)
  const _gdbs26 = GDBS.filter(r=>r[0]>='2026-01-01');
  const _gcT = _gdbs26.reduceRight((a,r)=>a!=null?a:(r[2]!=null?r[2]:null),null) || CURRENT.gdbC;
  const _gsT = _gdbs26.reduceRight((a,r)=>a!=null?a:(r[1]!=null?r[1]:null),null) || CURRENT.gdbS;
  const gcPrice = eur ? (_gcT * CURRENT.usdEur).toFixed(2) : _gcT.toFixed(2);
  const gsPrice = eur ? (_gsT * CURRENT.usdEur).toFixed(2) : _gsT.toFixed(2);
  const gcCur = eur ? "€" : "$";
  const gdb = [
    { label:"GDB.C", price:gcPrice, d:-0.0098, w:+0.0008, m:+0.0885, color:C.orange },
    { label:"GDB.S", price:gsPrice, d:+0.0026, w:+0.0004, m:+0.0952, color:C.blue },
  ];
  return(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:6}}>
        {cells.map((c,i)=>(
          <div key={i} style={{
            ...
            <div style={{fontSize:9,color:C.gray,marginBottom:2}}>{c.label}</div>
            <div style={{fontSize:12,fontWeight:800,color:clr(c.pnl)}}>
              {c.pnl>=0?"+":""}{cur}{fmtK(Math.abs(c.pnl))}
            </div>
            <div style={{
              display:"inline-block",fontSize:9,fontWeight:700,
              color:clr(c.pct),background:clr(c.pct)+"18",
              borderRadius:4,padding:"1px 4px",marginTop:1,
            }}>{fmtP(c.pct)}</div>
          </div>
        ))}
      </div>
...
function buildSections(L){
  ...
  return [
    ...
    {
      key:"cashdip", n:"Cash Dip", icon:"💰", color:"#06B6D4",
      totalUSD: cashDipUSD,
    ...
    {
      key:"matelas", n:"Cash Matelas", icon:"🏦", color:C.green,
...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync}){
  const cur = eur ? "€" : "$";
  const inv = 94064 * CURRENT.usdEur;
  const gcCur = eur ? "€" : "$";
  // GDB prices depuis GDBS (dernier point non-null) — cohérent avec onglet GDB
  const _gdbs26ov = GDBS.filter(r=>r[0]>='2026-01-01');
  const _gcTov = _gdbs26ov.reduceRight((a,r)=>a!=null?a:(r[2]!=null?r[2]:null),null) || CURRENT.gdbC;
  const _gsTov = _gdbs26ov.reduceRight((a,r)=>a!=null?a:(r[1]!=null?r[1]:null),null) || CURRENT.gdbS;
  const gcPrice = eur ? (_gcTov * CURRENT.usdEur).toFixed(2) : _gcTov.toFixed(2);
  const gsPrice = eur ? (_gsTov * CURRENT.usdEur).toFixed(2) : _gsTov.toFixed(2);
  ...
  return(
    <div>
      ...
      <div style={{
        ...
        {/* P&L 1D / 1S / 1M */}
        {(()=>{
          const rate = CURRENT.eurUsd;
          const pval = (v) => eur ? v : Math.round(v * rate);
          const cur2 = eur ? "€" : "$";
          const cells = [
            { label:"1 jour",    pnl:pval(-1008),  pct:-0.0031 },
            { label:"1 semaine", pnl:pval(+1935),  pct:+0.0061 },
            { label:"1 mois",    pnl:pval(+23736), pct:+0.0798 },
          ];
          return(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:1,background:C.border}}>
              {cells.map((c,i)=>(
                <div key={i} style={{background:C.bg2,padding:"10px 12px",textAlign:"center"}}>
                  <div style={{fontSize:9,color:C.gray,marginBottom:4}}>{c.label}</div>
                  <div style={{fontSize:14,fontWeight:800,color:clr(c.pnl)}}>
                    {hidden?"***":(c.pnl>=0?"+":"")+cur2+fmtK(Math.abs(c.pnl))}
                  </div>
                  <div style={{
                    fontSize:11,fontWeight:700,color:clr(c.pct),
      ...
      <GdbCompareChart eur={eur} EFF={EFF}/>
...
function PageAllocation({hidden, EFF}){
  ...
  const realD=CURRENT.alloc.map(a=>({v:a.pct/100,c:a.c,n:a.n,pct:a.pct,usd:a.usd}));
  const tgtD=CURRENT.alloc.map(a=>({v:a.tgt/100,c:a.c,n:a.n,pct:a.tgt}));

  /* sections data (moved from Portfolio) */
        ...
        <>
          ...
          <div style={{display:"flex",justifyContent:"space-around",alignItems:"center",marginBottom:10}}>
            <div style={{textAlign:"center"}}>
              <Donut data={realD} size={148} label="RÉEL" sub={"$"+fmtK(CURRENT.totalUSD)} ri={26}/>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:7,justifyContent:"center",marginBottom:14}}>
            {CURRENT.alloc.map((a,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{fontSize:10,color:C.text2}}>{a.n} <b style={{color:a.c}}>{a.pct.toFixed(1)}%</b></span>
              </div>
            ))}
          </div>
          {/* Liste ajustements */}
          {CURRENT.alloc.map((a,i)=>{
            const diff=a.pct-a.tgt,adj=Math.round((diff/100)*CURRENT.totalUSD);
            return(
              <div key={i} style={crd()}>
                ...
                <div style={{position:"relative",height:14,background:C.bg3,borderRadius:4,marginBottom:5,overflow:"hidden"}}>
                  ...
                  <div style={{position:"absolute",left:0,top:2,height:10,width:Math.min(a.pct/65*100,100)+"%",background:a.c,borderRadius:3}}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  <span style={{fontSize:10,color:C.gray}}>Réel <b style={{color:a.c}}>{a.pct.toFixed(1)}%</b></span>
          ...
          <div style={{background:C.bg2,borderRadius:12,padding:14,border:`1px solid ${C.border}`,marginTop:4}}>
            ...
            {CURRENT.alloc.filter(a=>Math.abs(a.pct-a.tgt)>1).map((a,i,arr)=>{
              const diff=a.pct-a.tgt,adj=Math.round((diff/100)*CURRENT.totalUSD);
              return(
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
            <div style={{flexShrink:0}}>
              {(()=>{
                ...
                return <Donut size={150} ri={30} label="TOTAL" sub={"$"+fmtK(sectionsTotal)}
                  data={SECTIONS.map(s=>({v:s.pct/100,c:s.color,n:s.n,pct:s.pct,usd:s.totalUSD}))}/>;
              })()}
            </div>
...
function PageStats({chartData, hidden=false, EFF}){
  ...
  const validPct = data?.pct?.filter(v=>v!=null)??[];
  const ttlPnl = validPnl.reduce((s,v)=>s+v,0);
  const avgPct = validPct.length?validPct.reduce((s,v)=>s+v,0)/validPct.length:0;
  const bestI  = data?.pct ? data.pct.reduce((bi,v,i)=>{if(v==null)return bi; return bi===-1||v>data.pct[bi]?i:bi;}, -1) : -1;
  const worstI = data?.pct ? data.pct.reduce((wi,v,i)=>{if(v==null)return wi; return wi===-1||v<data.pct[wi]?i:wi;}, -1) : -1;
      ...
      {data&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:14}}>
          {[
            ...
            ["Meilleur",bestI>=0?data.m[bestI]+" "+fmtP(data.pct[bestI]):"—",C.green],
            ["Pire",worstI>=0?data.m[worstI]+" "+fmtP(data.pct[worstI]):"—",C.red],
          ].map(([l,v,c])=>(
        ...
        <div style={{...crd(),marginBottom:14,padding:"10px 8px"}}>
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
              ...
              <tbody>
                {data.m.map((m,i)=>{
                  const eom=data.eom[i], bom=data.bom[i], pnl=data.pnl[i], pct=data.pct[i];
                ...
                <tr style={{borderTop:`1px solid ${C.border}`,fontWeight:800}}>
                  <td style={{padding:"5px 6px",color:C.text}}>TOTAL</td>
                  <td/><td/>
                  <td style={{padding:"5px 6px",textAlign:"right",color:C.teal}}>
                    {(()=>{const t=(data.inv||[]).filter(v=>v!=null).reduce((s,v)=>s+v,0); return t?msk((t>0?"+":"")+Math.round(t).toLocaleString("fr-FR")+"€",hidden):"—";})()}
                  </td>
                  <td style={{padding:"5px 6px",textAlign:"right",color:bclr(ttlPnl)}}>{msk((ttlPnl>=0?"+":"")+Math.round(ttlPnl).toLocaleString("fr-FR"),hidden)}</td>
                  <td style={{padding:"5px 6px",textAlign:"right",color:bclr(avgPct)}}>{fmtP(avgPct)}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
function PageGDB({chartData,hidden,EFF}){
  ...
  const gcToday  = gdbs2026[gdbs2026.length-1]?.[2] || src.gdbC;
  ...
  const gcQty   = gcToday>0 ? Math.round(src.crypto.total/gcToday) : 0;
  const gcFonds = Math.round(src.crypto.total);
  const actionsUSD = src.stocks.items.filter(x=>x.cat!=="Cash").reduce((s,x)=>s+x.val,0);
  const gsQty   = gsToday>0 ? Math.round(actionsUSD/gsToday) : 0;
  const gsFonds = Math.round(actionsUSD);

  const [benchTF, setBenchTF] = useState("ALL");

  const bench = (()=>{
    const TF_CUTS = {"1W":"2026-04-25","1M":"2026-04-02","MTD":"2026-05-01","YTD":"2026-01-01","1Y":"2026-05-02","ALL":"2023-01-01"};
    ...
    const gcLast = GC_FULL.reduceRight((a,r)=>a!=null?a:(r[1]!=null?r[1]:null),null);
...
/* ═══════════════════════════════════════════════════════════
   ...
   - Granularité journalière (YYYY-MM-DD) — pas mensuelle
   - Historise les 41 colonnes de l'onglet Chart Excel
   - Validation + confirmation visuelle
═══════════════════════════════════════════════════════════ */
    ...
    <Modal title="📸 Snapshot journalier" onClose={onClose}>
      {saved ? (
        <div style={{textAlign:"center",padding:"30px 0"}}>
          ...
          <div style={{fontSize:12,color:C.gray,marginTop:4}}>{dateInput}</div>
        </div>
      ) : (
        <>
          {/* Info source */}
