...
const CURRENT={
  ...
  alloc:[
    ...
    {n:"Cash",    pct:7.0,  tgt:15, c:"#50C878"},
  ],
};
...
function CloudKeyList({data}){
  ...
  // ── Vue détail d'une clé ────────────────────────────────────────────────
  if(selectedKey){
    const val   = data[selectedKey];
    const meta  = CLOUD_KEYS.find(function(k){ return k.key===selectedKey; });
    const label = meta ? meta.label : selectedKey;

    // Convertir la valeur en lignes affichables
    var rows = [];
    var headers = [];
    if(Array.isArray(val)){
      if(val.length===0){
        ...
        // Tableau de tableaux (DD, GDBS, GC_FULL...)
        headers = val[0].map(function(_,i){ return "Col "+(i+1); });
        rows = val;
      } else if(typeof val[0]==="object"){
        // Tableau d'objets (transactions, snapshots, portfolio.items...)
        headers = Object.keys(val[0]);
        rows = val.map(function(r){ return headers.map(function(h){ return r[h]!=null?String(r[h]):"—"; }); });
      }
    } else if(val && typeof val==="object"){
      // Objet simple (YF_MAP, bank.breakdown...)
      var entries = Object.entries(val);
      if(entries.length>0 && typeof entries[0][1]==="object"){
        // Objet d'objets (CRYPTO_MONTHLY, portfolio...)
        headers = ["Clé", "Valeur (JSON)"];
        rows = entries.map(function(e){ return [e[0], JSON.stringify(e[1]).slice(0,100)]; });
      } else {
        headers = ["Clé", "Valeur"];
        rows = entries.map(function(e){ return [e[0], String(e[1])]; });
      }
    }

    // Filtre
    var filtered = search
      ? rows.filter(function(r){ return r.some(function(v){ return String(v||"").toLowerCase().indexOf(search.toLowerCase())>=0; }); })
      : rows;
    ...
    return(
      <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          <button onClick={function(){ setSelectedKey(null); setSearch(""); }} style={{
            background:C.bg2,border:"1px solid "+C.border,borderRadius:8,
            padding:"6px 12px",color:C.text,fontSize:12,cursor:"pointer",fontWeight:700,
          }}>← Retour</button>
          <div>
            <div style={{fontSize:12,fontWeight:700,color:C.teal}}>{label}</div>
            <div style={{fontSize:9,color:C.gray,fontFamily:"monospace"}}>{"kv/"+selectedKey+" — "+(Array.isArray(val)?val.length+" lignes":Object.keys(val||{}).length+" clés")}</div>
          </div>
        </div>
        ...
        <input value={search} onChange={function(e){ setSearch(e.target.value); }} placeholder="Filtrer..."
          style={{width:"100%",background:C.bg2,border:"1px solid "+C.border,borderRadius:8,
            padding:"7px 10px",color:C.text,fontSize:16,marginBottom:10,outline:"none"}}/>

        {rows.length===0
          ? <div style={{textAlign:"center",padding:"20px",color:C.gray,fontSize:12}}>Vide ou format non affichable</div>
        ...
        {filtered.length>100&&(
          <div style={{fontSize:10,color:C.gray,textAlign:"center",marginTop:6}}>100 premiers sur {filtered.length}</div>
        )}
      </div>
    );
  }
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {CLOUD_KEYS.map(function(item){
        ...
        return(
          <div key={item.key} onClick={function(){ if(!empty){ setSelectedKey(item.key); setSearch(""); } }}
            style={{
            ...
            <div style={{textAlign:"right",display:"flex",alignItems:"center",gap:8}}>
              ...
              {!empty && <span style={{fontSize:14,color:C.teal}}>›</span>}
            </div>
          </div>
        );
      })}
